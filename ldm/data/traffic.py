...
class TrafficBase(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if 'train' in path:
            textpath = '/root/autodl-tmp/datasets/traffic/train/' + 'text/' + path.split('/')[-1].split('.')[0] + '.txt'
        else:
            textpath = '/root/autodl-tmp/datasets/traffic/validation/' + 'text/' + path.split('/')[-1].split('.')[0] + '.txt'
